...
export default function UserProfileScreen() {
  ...
  const navigateToFollowers = () => {
    if (!profile) return;
    console.log(`[${new Date().toISOString()}] Navigating to followers for ID: ${profile._id}`);
    router.push(`/profile/followers?id=${profile._id}`);
  };

  const navigateToFollowing = () => {
    if (!profile) return;
    console.log(`[${new Date().toISOString()}] Navigating to following for ID: ${profile._id}`);
